...
// Optimisations perf :
//   • Labels dessinés sur canvas (fillText) — plus de manipulation DOM
//   • Zoom debounce via requestAnimationFrame
...
const CircularTree = (function () {
  ...
  // RAF debounce zoom
  let _rafZoom        = null;
  let _pendingZoomVal = null;
  let _pendingPivotX  = null;
  let _pendingPivotY  = null;
  ...
  async function init(container, onSelectFn) {
    ...
    // Données
    try { _map = await api.getSosaMap(); }
    catch (e) { console.error('CircularTree: impossible de charger la carte sosa', e); }
  ...
  function redraw() {
    ...
    // Si init() est encore en cours (démarrage sur fiche), on ne fait rien :
    // init() appellera _draw() lui-même quand il sera prêt.
  }
  ...
  function _scheduleZoom(targetZoom, px, py) {
    _pendingZoomVal = Math.min(ZOOM_MAX, Math.max(ZOOM_MIN, targetZoom));
    _pendingPivotX  = px;
    _pendingPivotY  = py;
    if (!_rafZoom) {
      _rafZoom = requestAnimationFrame(() => {
        _rafZoom = null;
        _applyZoom(_pendingZoomVal, _pendingPivotX, _pendingPivotY);
  ...
  function _onWheel(e) {
    ...
    const base  = _pendingZoomVal !== null ? _pendingZoomVal : _zoomLevel;
    _scheduleZoom(base + delta, e.clientX, e.clientY);
  }
  ...
  // ── Centrage / scroll area ────────────────────────────────────────────────

  function _centerView() {
    if (!_scrollEl) return;
    const sw = _scrollEl.clientWidth, sh = _scrollEl.clientHeight;
    _scrollEl.scrollLeft = _size > sw ? (_size - sw) / 2 : 0;
    _scrollEl.scrollTop  = _size > sh ? (_size - sh) / 2 : 0;
  }

  function _updateScrollArea() {
    const sa = document.getElementById('tree-scroll-area');
    if (!sa || !_scrollEl) return;
    sa.style.width  = Math.max(_scrollEl.clientWidth,  _size) + 'px';
    sa.style.height = Math.max(_scrollEl.clientHeight, _size) + 'px';
  }
  ...
  function _makeCanvas(parent, id) {
    ...
    c.id = id; c.style.cssText = 'position:absolute;left:0;top:0;';
  ...
  function _draw() { _drawNodes(); _drawLabels(); }
  ...
  // ── Dessin des labels (canvas, sans DOM) ─────────────────────────────────
  // Perf : ~800 DOM nodes évités — remplacés par fillText natif canvas

  function _drawLabels() {
    _ctx.save();
    _ctx.fillStyle = '#333'; _ctx.textAlign = 'center'; _ctx.textBaseline = 'middle';

    // Centre (cujus) — deux lignes
    if (_map[1]) {
      const p  = _map[1];
      ...
      _ctx.fillText(fn,          _mid, cy - fs * 0.65);
      _ctx.fillText(p.nom || '', _mid, cy + fs * 0.65);
    }

    // Layer 1 (parents)
    const fs1 = Math.max(7, Math.min(11, Math.round(_rad * 0.3)));
    _ctx.font = `${fs1}px Arial`;
    if (_map[2]) _ctx.fillText(_shortName(2), _mid, _mid - _rad * 1.5 + 7);
    if (_map[3]) _ctx.fillText(_shortName(3), _mid, _mid + _rad * 1.5 - 5);

    // Layer 2 (grands-parents, 45°)
    ...
    ].forEach(([id, x, y, angle]) => {
      if (!_map[id]) return;
      _ctx.save();
      _ctx.translate(x, y); _ctx.rotate(angle);
      _ctx.font = `${fs2}px Arial`;
      _ctx.fillText(_shortName(id), 0, 0);
      _ctx.restore();
    });
  ...
  function _onMove(evt) {
    ...
    _getDescendants(id).forEach(d => _drawSegmentById(d,  'lightblue',  0.85));

    if (layer === 0) {
      _hctx.globalAlpha = 1; _hctx.fillStyle = 'orange';
      _hctx.beginPath(); _hctx.arc(_mid, _mid, _rad, 0, 2 * Math.PI);
      _hctx.fill(); _hctx.globalAlpha = 1;
    } else {
  ...
  function _onClick(evt) {
    ...
    if (!hit) return;
    const p = _map[hit.id];
    if (p && p.id && _onSelect) _onSelect(p.id);
  }
  ...
  function _drawSegmentById(id, color, alpha) {
    if (id === 1) {
      _hctx.globalAlpha = alpha; _hctx.fillStyle = color;
      _hctx.beginPath(); _hctx.arc(_mid, _mid, _rad, 0, 2 * Math.PI);
      _hctx.fill(); _hctx.globalAlpha = 1; return;
    }
    const layer = Math.floor(Math.log2(id));
    _drawSegment(_hctx, layer, id - Math.pow(2, layer), color, alpha);
  }

  function _getDescendants(id) {
    const res = []; let cur = id;
    while (cur > 1) { cur = Math.floor(cur / 2); res.push(cur); }
    return res;
  }
  ...
  function _showTooltip(html, px, py) {
    ...
    tip.style.left = (px + 18) + 'px'; tip.style.top = (py + 14) + 'px';
  }
  ...
  function _tooltipHtml(id) {
    ...
    function fmt(s) {
      ...
      return String(s)
        ...
        .replace(/\bABT\b/gi,'v.').replace(/\bBEF\b/gi,'av.').replace(/\bAFT\b/gi,'ap.');
    }
    function line(lbl, date, ville) {
      const l = [fmt(date), ville].filter(Boolean).join(' – ');
