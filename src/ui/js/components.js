...
const DEPT_NOMS = {
  ...
  '17':'Charente-Maritime','18':'Cher','19':'Corrèze','20A':'Corse-du-Sud',
  '20B':'Haute-Corse','21':'Côte-d\'Or','22':'Côtes-d\'Armor','23':'Creuse',
...
function deptLabel(num) {
  if (!num) return '';
  const nom = DEPT_NOMS[num];
  return nom ? `${num}, ${nom}` : num;
}

function formatPlace(lieu) {
  if (!lieu) return null;

  // Partie droite : ville (dept) / ville / (dept) / dept seul
  const dept = deptLabel(lieu.dept_num);
  let right = '';
  if (lieu.ville && dept) right = `${lieu.ville} (${dept})`;
  else if (lieu.ville)    right = lieu.ville;
  else if (dept)          right = dept;

  // Assemblage : adresse | right, ou l'un seul si l'autre est absent
  let result = '';
  if (lieu.adresse && right) result = `${lieu.adresse} | ${right}`;
  else if (lieu.adresse)     result = lieu.adresse;
  else                       result = right;

  return result || (lieu.brut || null);
}
...
function renderPersonHeader(person) {
  ...
  if (person.sosa != null) {
    ...
    targetBtn.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"/><circle cx="12" cy="12" r="5"/><circle cx="12" cy="12" r="1" fill="currentColor" stroke="none"/></svg>';
    targetBtn.addEventListener('click', () => {
      goHome();
      CircularTree.highlight(person.sosa);
    });
